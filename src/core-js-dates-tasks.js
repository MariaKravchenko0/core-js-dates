...
function getNextFriday(date) {
  ...
  let daysTillFriday = (5 - currDayOfWeek + 7) % 7;
  if (daysTillFriday === 0) daysTillFriday = 7;

  const nextFriday = new Date(date);
  nextFriday.setDate(nextFriday.getDate() + daysTillFriday);
  return nextFriday;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 1; day <= amountOfDays; day += 1) {
    ...
    if (dayOfTheWeek === 0 || dayOfTheWeek === 6) weekendCount += 1;
  }
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber = Math.ceil((daysSinceStartOfYear + firstDayOfYear) / 7);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currDate <= endDate) {
    for (let i = 0; i < countWorkDays && currDate <= endDate; i += 1) {
      schedule.push(stringifyDate(currDate));
